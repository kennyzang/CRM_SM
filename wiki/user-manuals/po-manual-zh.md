...
PO 始终关联至状态为**赢单（Won）**的商机，并自动从关联报价单中继承财务信息。

### 1.1 入口

| 入口 | 路径 |
|------|------|
| 独立列表 | 左侧导航 → **采购订单（Purchase Order）** |
...
显示新建表单中的所有字段，自动填充字段（商机、客户、货币、总金额）以只读方式呈现。
...
```
商机（赢单）→ 创建 PO → 填写 PO 编号 + 上传 PO 文件 → 保存 → 提交审批 → PO 生效
```

1. 商机须处于**赢单（Won）**状态（或创建 PO 的动作将其推进至赢单）。
2. 填写必填字段：PO 编号、日期、报价单、交易类别、PO 文件。
3. 系统从所选报价单自动填充客户、商机、货币和总金额。
4. 保存 PO 记录。
5. 通过右侧面板的工作流提交审批。
6. 审批通过后，PO 状态变为**生效（Active）**，解锁销售订单/合同的创建。
...
**PO 编号**是客户在其正式 PO 文件上标注的参考编号，必须与文件一致。此编号用于发票开具和客户付款对账。
...
选定报价单后，货币和总金额将锁定以与报价单保持一致。如数值有误，请先修改源报价单，再重新关联。

---

## 6. 常见问题与注意事项

**问：为什么建议从商机的 PO 选项卡创建，而不是从 PO 列表页创建？**
从商机 PO 选项卡创建时，上下文已预设完成，减少数据录入错误。从列表页创建时，需要手动选择报价单，容易出错。

**问：选择报价单后，商机字段为空或显示有误，怎么办？**
请确认所选报价单已正确关联至商机。若报价单创建时未关联商机，请先修改报价单再重试。

**问：一个 PO 可以上传多个文件吗？**
可以，PO 文件上传字段通常支持多附件。请将所有相关文件（如原始 PO、修订版）一并上传。
