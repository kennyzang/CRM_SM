...
All fields confirmed via Playwright MCP DOM inspection (2026-04-22). Source: `pages/registry/lead.fields.ts`.
Video-confirmed fields marked with [V] (from "Contact & Lead Creation" and "Lead Management" videos, 2026-04-24).
...
| File | Class | Purpose |
|------|-------|---------|
| `pages/LeadCreatePage.ts` | `LeadCreatePage` | Legacy page object with detailed field methods |
| `pages/LeadFormPage.ts` | `LeadFormPage` | New FormEngine-based page object |
| `pages/registry/lead.fields.ts` | `LEAD_SCHEMA` | Field registry (single source of truth) |

## Known Issues

1. **CRM typo "teaxtarea"**: The Details field uses `comp-fd_remark--teaxtarea` (not "textarea"). See [[pitfall-log]].
...
The CRM currently displays Chinese labels (e.g., "线索名称" for "Lead Name"). All Chinese text in the UI is considered a **defect** — the system's official language is English.
...
- ☑ "If the lead creator is a member of the lead queue"
- ☑ "If the lead creator is an admin of the lead queue"
