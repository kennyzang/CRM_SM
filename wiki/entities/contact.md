...
Source: `pages/registry/contact.fields.ts`.
...
3. **window.close() pitfall**: When creating a contact via a popup window, `window.close()` may interfere with Playwright. See [[pitfall-log]].

## Language Note

Chinese labels in the Contact UI (e.g., "联系人姓名" for "Contact Name") are **defects**. The system official language is English.
