...
> **Externally (customer/documents) always call it P&L**, not PL. PL is only the abbreviation in directory/files/code.
> - Customer perspective: P&L (Profit & Loss)
> - Code/files: `pl.md`, `pl.spec.ts`, `mk_km_ltc_pl.json` (technical abbreviations remain unchanged)
> - Wiki text: First occurrence write full name "P&L (Profit & Loss)", subsequently use P&L uniformly
...
3. **Margin calculation**: Margin % = Expected Profit / Total Selling Price x 100
...
- **Level 2 (Child): Resource Role** -- Lookup from "Service Master" (e.g., "Senior Engineer", "Project Manager")
  ...
  - Formula: **Man Days x Rate per Day = Total**
...
| Margin (%) | Expected Profit / Total Selling Price x 100 |
...
| # | Bug | Video | Status |
|---|-----|-------|--------|
| 1 | **Software products not selectable in P&L** -- only Principal = Securemetric products can be selected in Software section; third-party software products are filtered out | P&L Creation | Known |
| 2 | **Opportunity products don't auto-fill in P&L** -- when selecting an opportunity, the product line should auto-populate with products from the opportunity, but stays empty | P&L Creation | Known |
| 3 | **Product carryover misses software items** -- when creating new P&L from opportunity with 3 products, only 2 carry over (software product is missing) | New P&L management | Known |
| 4 | **Reimbursement has incorrect selling price field** -- reimbursement should only have cost (pass-through), but the form shows selling price too | P&L Creation | Known |
| 5 | **Third-party product filter not scoped to entity/opportunity** -- when selecting third-party software, shows ALL products in system instead of filtering by entity (SCMY) and opportunity | P&L Creation | Known |
| 6 | **Services lacks markup field** -- unit price equals cost, no markup can be applied, meaning 0% margin always | New P&L management | Known |
| 7 | **Reimbursement total cost = 0 when quantity not specified** -- calculation error: total cost should be unit cost x quantity, but shows 0 until quantity is entered | New P&L management | Known |

### Financial Calculation Clarifications [A]
- **Margin formula**: `Expected Profit / Total Selling Price x 100` (this is Gross Margin, NOT Markup)
- **Markup formula**: `(Markup Price - List Price) / List Price x 100`
...
- Example: actual flight cost 300, sell at 400 -> profit 100, margin 25%

### Approval Workflow Conditions [A]
1. If **any line item margin < target margin** -> route to Sales Team Supervisor
2. If **all line item margin >= target margin** -> auto-approve
...
| MARGIN | Green text (or red if below target) | Calculated as (Profit / Selling Price) x 100 -- **standard Gross Margin** |
...
| QTY | Quantity |
| TOTAL PRICE | PRICE x QTY |
| COST | Total cost for this line |
| PROFIT | TOTAL PRICE - COST (green if positive) |
| MARGIN | (PROFIT / TOTAL PRICE) x 100 -- **red warning icon if below Target** |
...
1. **Margin calculation**: System uses `(Profit / Selling Price) x 100`, which is standard **Gross Margin**
...
|-------|-------|----------|--------|
| TC-011 | P&L Financial Summary - Margin Calculation Accuracy (Markup vs Gross Margin) | P1 | Not Tested |
| TC-012 | P&L Global Discount - Toggle ON and OFF Behavior | P1 | Not Tested |
| TC-013 | P&L Multi-User Collaboration - Sales Rep to Solution Architect Workflow | P1 | Not Tested |
| TC-014 | P&L Approval Workflow - Margin-Based Routing Conditions | P1 | Not Tested |
| TC-015 | P&L Product Selection - Modal Filters and Category Scope | P2 | Not Tested |
| TC-016 | P&L Details - Multi-Year Breakdown (Year 1-3 RENEW) | P2 | Not Tested |
