...
| **预期结果** | - MARGIN < TARGET 时，显示红色感叹号警告图标<br>- MARGIN ≥ TARGET 时，显示绿色文字<br>- 利润率计算公式：(PROFIT / COST) × 100% |
...
| **测试步骤** | 1. 记录每个产品行的 TOTAL PRICE 和 COST<br>2. 计算所有产品行的 TOTAL PRICE 总和<br>3. 计算所有产品行的 COST 总和<br>4. 验证 Total Revenue = 所有 TOTAL PRICE 之和<br>5. 验证 Total Cost = 所有 COST 之和<br>6. 验证 Total Profit = Total Revenue - Total Cost<br>7. 验证 Margin = (Total Profit / Total Cost) × 100% |
...
| **测试步骤** | 1. 选择一个产品行，记录其 PROFIT 和 COST 值<br>2. 手动计算 (PROFIT / COST) × 100%<br>3. 验证与 MARGIN 列显示的值一致<br>4. 手动计算 (PROFIT / TOTAL PRICE) × 100%（标准毛利率）<br>5. 验证与 MARGIN 列显示的值不同 |
| **预期结果** | - 系统使用 (PROFIT / COST) × 100% 计算 Margin（Markup）<br>- 与标准毛利率 (PROFIT / TOTAL PRICE) × 100% 结果不同<br>- 这在文档中已说明为已知行为 |
