...
Output:
    test-cases-zh.xlsx      — Chinese version (base)
    test-cases-en.xlsx      — English version (base)
    test-cases-zh-v2.xlsx   — Chinese version v2 (extended)
    test-cases-en-v2.xlsx   — English version v2 (extended)
"""
...
def main():
    jobs = [
        ('test-cases-zh.json',    'test-cases-zh.xlsx',    '测试用例 (中文)'),
        ('test-cases-en.json',    'test-cases-en.xlsx',    'Test Cases (English)'),
        ('test-cases-zh-v2.json', 'test-cases-zh-v2.xlsx', '测试用例 v2 (中文)'),
        ('test-cases-en-v2.json', 'test-cases-en-v2.xlsx', 'Test Cases v2 (English)'),
    ]

    for json_name, xlsx_name, title in jobs:
        json_path = BASE_DIR / json_name
        if not json_path.exists():
            print(f"⚠️  Skipped (not found): {json_name}")
            continue
        data = load_json(json_name)
        create_excel(data, BASE_DIR / xlsx_name, title)
